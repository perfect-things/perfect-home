<button
	title="Settings"
	class="icon-btn btn-settings {isVisible ? '' : 'hidden'}"
...
<div class="settings-pane {isVisible ? '' : 'hidden'}" tabindex="-1">
	<h1>Settings</h1>
	<div class="settings-form">
		...
		<div class="settings-row">
			<button type="button" class="btn btn-clear danger"
...
function toggle (forceTo) {
	isVisible = (typeof forceTo === 'boolean') ? forceTo : !isVisible;
	if (forceTo instanceof Event) {
		const btn = forceTo.target.closest('button');
		btn.focus();
	}
}
